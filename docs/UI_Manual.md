...
## Phase 9: Human Resources & Payroll

### 1. Complete HRMS DocType & Field Reference Manual
For exhaustive, field-by-field instructions, explanations, and real-world business examples for every single DocType in the **Frappe HRMS (`hrms`) app**, refer directly to the dedicated manual:
👉 **[HRMS UI & DocType Field Reference Manual (manual_ui.md)](file:///home/anonymous/projects/erp_next/frappe-bench/apps/jahan_kodak/docs/manual_ui.md)**

### 2. Executing Payroll Entry Dual-Approval Workflow
**How to do it from the UI:**
1. Navigate to **Payroll -> Payroll Entry -> Add Payroll Entry**.
2. Select **Posting Date**, **Company** (`Jahan Kodak`), **Payroll Frequency** (`Monthly`), and period dates.
3. Filter by **Branch** (e.g. `Kabul Center`) if executing branch-isolated payroll.
4. Click **Get Employees**. The table auto-populates all eligible active employees.
5. Click **Actions -> Submit for HR Approval**. Status transitions to `Pending HR Approval`.
6. **HR Manager** reviews attendance and leave deductions, then clicks **Approve HR Review**. Status transitions to `Pending Finance Approval`.
7. **Finance Manager** verifies financial totals and clicks **Approve Payroll**. The system submits salary slips and posts GL entries to `Payroll Payable - JK`.

---

## Phase 10: Reports & Dashboards

This section explains how executives, managers, and accountants access and use custom management reports and interactive dashboards in ERPNext.

### 1. Accessing & Running Custom Management Reports

#### A. Branch Profitability Report
**How to access from UI:**
1. Type **Branch Profitability** into the global search bar at the top of the screen and press **Enter**.
2. Alternatively, navigate to **Accounts -> Financial Statements -> Branch Profitability**.

**How it works & Filter usage:**
- **Company:** Set to `Jahan Kodak`.
- **From Date / To Date:** Select the accounting period (e.g. `2026-07-01` to `2026-07-31`).
- **Cost Center / Branch:** Leave blank to view all branches side-by-side, or select a specific branch (e.g. `Kabul Center - JK`).

**What the columns mean:**
- **Gross Sales:** Total sales revenue before returns.
- **Sales Returns:** Credit notes / returns deducted.
- **Net Revenue:** Actual net sales (`Gross Sales - Returns`).
- **COGS:** Direct cost of inventory sold.
- **Gross Profit:** Net revenue minus inventory cost (`Net Revenue - COGS`).
- **Operating Expenses:** Store overheads, utility bills, and salaries allocated to the branch.
- **Loyalty Expense:** Customer rewards redeemed at this store (`Loyalty Program Expense - JK`).
- **Net Profit:** Real bottom-line profitability for the store location (`Gross Profit - Operating Expenses - Loyalty Expense`).

---

#### B. Employee Performance Report
**How to access from UI:**
1. Search **Employee Performance** in the global search bar and select **Employee Performance List/Report**.
2. Or navigate to **Retail / HR -> Reports -> Employee Performance**.

**How it works & Filter usage:**
- **From Date / To Date:** Set date range for the evaluation period.
- **Branch:** Filter by physical store branch (e.g. `Shahr-e-Naw`).

**What the columns mean:**
- **Total Sales Invoices:** Total number of customer transactions processed by the employee.
- **Total Sales (AFN):** Total gross sales revenue generated by the cashier.
- **Avg Order Value (AFN):** Average basket size (`Total Sales / Total Invoices`). High average order values indicate effective upselling!
- **Days Present:** Verified shift attendance days logged in the biometric/attendance system.

---

#### C. Top Selling & Slow-Moving Items Report
**How to access from UI:**
1. Search **Top Selling and Slow Moving Items** in the global search bar.
2. Or navigate to **Stock -> Stock Reports -> Top Selling and Slow Moving Items**.

**How it works & Filter usage:**
- **Warehouse:** Filter by specific warehouse (e.g. `Karteh Naw - JK`) or leave blank for company-wide stock.
- **Filter By Velocity:** Select **Top Selling** to rank items by highest sales volume, or select **Slow Moving** to identify dead inventory sitting in stores.

**What the columns mean:**
- **Stock On Hand:** Total physical inventory balance sitting in the warehouse (`actual_qty`).
- **Reserved Qty:** Inventory allocated/committed to open orders or transfers (`reserved_qty`).
- **Available Qty:** Net unreserved quantity available for immediate sale (`Stock On Hand - Reserved Qty`).

**Business Decisions based on this report:**
- **Top Selling:** Re-order items immediately if **Available Qty** is low before running out of stock (prevents lost revenue).
- **Slow Moving:** Identify items with high **Available Qty** but zero recent sales. Mark down prices, run promotional bundles, or transfer dead stock from low-traffic stores to high-traffic central stores.

---

### 2. Creating Custom Number Cards & Charts for Dashboards

ERPNext allows executives to turn any report into visual charts and dashboard numbers without writing code.

**How to create a Dashboard Number Card from UI:**
1. Search **Number Card List** -> Click **Add Number Card**.
2. **Label:** E.g., `Today's Kabul Branch Sales`.
3. **DocType:** `Sales Invoice`.
4. **Function:** `Sum` -> **Aggregate Field:** `grand_total`.
5. **Filters:** Add condition `company = Jahan Kodak`, `posting_date = Today`, `docstatus = 1`.
6. Click **Save**.


---

### 3. Jahan Kodak Standard Executive Dashboards & Statistics

The `jahan_kodak` app comes bundled with pre-configured executive dashboards and KPI statistics that are automatically exported to version control (`fixtures/`).

#### Included Executive Statistics (Number Cards):
- **Monthly POS Sales (AFN):** Real-time aggregate revenue generated across all retail POS terminals.
- **Total Stock Valuation (AFN):** Live valuation of all inventory held in central and branch warehouses.
- **Active Staff Count:** Active headcount logged in the HR system.
- **Pending Stock Transfer Requests:** Count of unapproved stock movements requiring manager review.
- **Outstanding Receivables (AFN):** Total unpaid customer sales invoice balances.
- **Total Loyalty Points Issued:** Customer loyalty rewards points balance issued across branches.

#### Visual Executive Charts:
- **Branch POS Sales Distribution:** Bar chart comparing revenue contributions per physical store branch (`cost_center`).
- **Daily POS Sales Trend:** Line chart tracking daily revenue velocity over time.
- **Stock Valuation By Warehouse:** Donut chart breaking down inventory value across retail stores and central stores.


#### How to Access the Dashboard on your Local Computer:

1. **Method 1: Global Search Bar (Awesomebar)**
   - Click the search bar at top center (or press `Ctrl + K`).
   - Type **Jahan Kodak Executive Dashboard** -> Click to open.
   - Or type **Jahan Kodak Executive** -> Click the workspace page to open.

2. **Method 2: Left Sidebar Navigation**
   - Click **Workspaces** in the left sidebar.
   - Look under **Modules** and click **Jahan Kodak Executive**.

3. **Method 3: Direct Web URLs**
   - **Executive Dashboard View:** [http://development.localhost:8000/app/dashboard-view/Jahan%20Kodak%20Executive%20Dashboard](http://development.localhost:8000/app/dashboard-view/Jahan%20Kodak%20Executive%20Dashboard)
   - **Executive Workspace View:** [http://development.localhost:8000/app/jahan-kodak-executive](http://development.localhost:8000/app/jahan-kodak-executive)

#### How to Sync Dashboards to Live Server via Git:
1. **On Development Machine:**
   ```bash
   git add .
   git commit -m "Add executive dashboards and statistics fixtures"
   git push origin main
   ```
2. **On Live Production Server:**
   ```bash
   cd frappe-bench/apps/jahan_kodak
   git pull origin main
   bench --site <your-live-site-name> migrate
   ```

---

### 4. Biometric Attendance Integration Guide

For setting up biometric devices (ZKTeco K40, MB20, MB560-VL) across retail store branches linked to Hostinger VPS, see the complete step-by-step technical guide:
📄 [biometric_integration_guide.md](file:///home/anonymous/projects/erp_next/frappe-bench/apps/jahan_kodak/docs/biometric_integration_guide.md)
